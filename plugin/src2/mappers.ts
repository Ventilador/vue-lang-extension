...
export type Options = Pick<Utils, 'calculatePosition' | 'shouldRemap' | 'getLineAndChar'>;
...
function createMappersInternal({ calculatePosition, shouldRemap, getLineAndChar }: Options) {
    ...
    function outFileTextChanges(fileName: string, fileTextChanges: FileTextChanges): FileTextChanges {
        if (shouldRemap(fileTextChanges.fileName)) {
            return {
                fileName: fileName,
    ...
    function outRenameLocation(fileName: string, rename: RenameLocation): RenameLocation {
        if (shouldRemap(rename.fileName)) {
            return {
                fileName: fileName,
