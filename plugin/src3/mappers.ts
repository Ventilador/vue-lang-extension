...
export type Options = Pick<Utils, 'calculatePosition' | 'isVueFile' | 'getLineAndChar' | 'toTsFile' | 'toVueFile'>;
const patched = Symbol('patched');
export function createMappers(utils: Options): Mappers {
    if ((utils as any)[patched]) {
        return (utils as any)[patched];
    }
    return (utils as any)[patched] = createMappersInternal(utils)
}
function createMappersInternal({ calculatePosition: calcPos, isVueFile, getLineAndChar, toTsFile, toVueFile }: Options) {
    ...
    return mappers;
    function calculatePosition(file: string, val: any, positive: boolean = true) {
        return positive ? calcPos({
            from: toTsFile(file),
            to: toVueFile(file)
        }, val) : calcPos({
            from: toVueFile(file),
    ...
    function outFileTextChanges(fileName: string, fileTextChanges: FileTextChanges): FileTextChanges {
        if (isVueFile(fileTextChanges.fileName)) {
            return {
                fileName: fileName,
    ...
    function outSignatureHelpItems(file: string, val: SignatureHelpItems): SignatureHelpItems {
        if (isVueFile(file)) {
            return {
                applicableSpan: outTextSpan(file, val.applicableSpan),
